...
@section('content')
    <div class="row" id="">
        <div class="col-lg-12">
            ...
            @if(isset($user_edit_message))
                <div class="flash-msg {{$user_edit_cls ?? 'flash-info'}} rounded-none">
                    <a href="#" class="close">×</a>
                    <div class="text-lg"><strong>{{ $user_edit_msgTitle ?? 'Info!'}}</strong></div>
                    <p>{{ $user_edit_message ?? 'Info!' }}</p>
                    <div class="text-base">{!! $user_edit_message2 ?? '' !!}</div>
                </div>
            @endif
