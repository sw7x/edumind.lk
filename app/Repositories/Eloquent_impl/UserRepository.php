...
private function getNotificationCategorysBy(array $where)
{
    if (empty($where)) {
        return false;
    }

    return $this->getNotificationCategoryDataFromDbToIsDataExistMethod($where)->toArray();
}


public function deleteCustomer($customerId, Customer $customerObj)
{
    $this->getAppLoggerService()->log(\Laminas\Log\Logger::INFO, "Entering Method `".__METHOD__."`");
    $this->getAppLoggerService()->log(\Laminas\Log\Logger::INFO, "Processing Method `writeCustomerData`");

    return $this->writeCustomerData(self::METHOD_DELETE, $customerObj);
}


private function writeCustomerData($method, $customerObj){
    try  {
        $this->getDbAdapter()->getDriver()->getConnection()->beginTransaction();

        $customerId = $customerObj->getId();

        $this->getAppLoggerService()->log(\Laminas\Log\Logger::DEBUG, "Processing Method `" . __METHOD__ . ":" . $method . "` with id :" . $customerId);
        $this->getAppLoggerService()->log(\Laminas\Log\Logger::INFO, "Processing Method `" . __METHOD__ . ":" . $method . "`");


        $dbStrctDataSet = ['is_deleted' => 'Y'];

        $status = $this->getTableGateway()->update(
            $dbStrctDataSet,
            ['customer_id' => $customerId]
        );
                //        $this->writeCustomerHistoryData($customerObj, $method);

        $newCustomerObj = $customerObj;
        
    }catch{
       $newCustomerObj = null;

       $this->getDbAdapter()->getDriver()->getConnection()->rollback();
       throw new Exception("Fail to add Customer with error ".$e->getMessage(),2019040110);
    }
